...
- the [command line](/:product/:version?/toolbelt/_containers-list) (Toolbelt)
...
You can use the `cx containers list` command to check on the status of your application’s containers. Please read the [how-to-guide](/:product/:version?/toolbelt/_containers-list) for more details.
...
2. Use the `cx containers attach` command to connect to detailed output (`STDOUT`) from the container ([guide here](/:product/:version?/toolbelt/_containers-attach))
3. Use the `cx run` command with the `--interactive` argument and specify the name of the container to run commands inside it ([guide here](/:product/:version?/toolbelt/_run)) It’s useful to be “attached” to a container while running commands to track the effects.
