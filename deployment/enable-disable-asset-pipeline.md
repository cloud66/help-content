...
[Asset Pipeline Compilation](https://guides.rubyonrails.org/asset_pipeline/) (APC) is the process that Rails uses to consolidate and optimise assets like JavaScript and CSS, and to automate the compilation of embedded languages. For more information about this.
...
{% callout type="warning" title="webpack requires NODE_ENV" %}
If your app uses webpack, ensure you define your NODE_ENV [environment variable](/docs/build-and-config/env-vars). If it is not set it can interfere with the compilation process (both standard and dedicated).
{% /callout %}
...
You can use a [manifest file](/docs/manifest/building-a-manifest-file#manifest-tutorial) to enable/disable the APC using the following parameters with a true or false:
