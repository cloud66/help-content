...
If your app uses webpack, ensure you define your NODE_ENV [environment variable](/:product/:version?/build-and-config/env-vars). If it is not set it can interfere with the compilation process (both standard and dedicated).
</Callout>
...
You can use a [manifest file](/:product/:version?/manifest/building-a-manifest-file#manifest-tutorial) to enable/disable the APC using the following parameters with a true or false:
