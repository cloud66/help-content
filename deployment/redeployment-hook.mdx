...
Cloud 66 offers several methods for achieving continuous integration. Please read our [continuous integration guide](/:product/:version?/deployment/continuous-deployment) to compare and select the ideal method to suit your requirements.
...
[Deployment profiles](/:product/:version?/deployment/deploy-profiles) allow you to customise the deployment process depending on your requirements - for example deploying some components and not others. You can also call deployment profiles directly via redeployment hooks using the following syntax:
...
To do this: access your [Application settings](/:product/:version?/toolbelt/toolbelt#settings-set) via the toolbelt and set **continuous.deploy** to _true_.
...
If you are manually invoking redeployments you should consider using [Cloud 66 Toolbelt](/:product/:version?/toolbelt/toolbelt#redeploy-your-stack) instead, as it has additional features!
</Callout>
</PerProduct>
