...
Cloud 66 uses Ubuntu as the operating system for all our managed servers. (You can check [which versions we currently support here](/:product/:version?/specs-and-policies/technical-specifications).) 
...
The best way to add custom packages to your application is using deploy hooks. If you're not already familiar with using deploy hooks, please follow our [tutorial](/:product/:version?/deploy-hooks/deploy-hooks). 
...
In this example we install the `zip` package to our `Production` environment using a [command hook type](/:product/:version?/deploy-hooks/deploy-hooks#hook-fields-commands), targeting all our servers and we specify that this should **only be done when we build the server**, using the `apply_during` option: 
