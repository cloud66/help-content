...
3. If the servers appear to be functional, try to connect to them [via SSH](/docs/servers/ssh-to-server) using the Cloud 66 Toolbelt (if this fails, try to connect via the [manual](/docs/servers/ssh-to-server#manual-shell-access) method). 
4. If you still can't connect, check that your firewall settings on your [cloud provider](/docs/servers/ssh-to-server#cloud-provider-s-firewall-api-has-delay) are not blocking access to the servers for port 22 (which is the ssh connection port).
...
2. If there is a version conflict check that your components have not been version-locked in your [Manifest file](/docs/manifest/building-a-manifest-file). Generally we will deploy the latest canonical versions of components, unless we are instructed not to by a manifest or other setting.
...
4. Check which version of Ubuntu your servers are using (you can find this on the detail page for any server, under the **Overview** tab). Older servers may be running version 16.04 or even 14.04, both of which are now deprecated and this can cause issues. Consider [upgrading](/docs/servers/applying-upgrades) these servers. 
...
If a deployment depends on a process to finish running - particularly data or file migrations - it's possible for that process to timeout. This can happen (rarely) with the Ruby [asset precompile process](/docs/deployment/enable-disable-asset-pipeline), for example. 
