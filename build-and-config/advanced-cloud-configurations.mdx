...
You can use your instance profiles via Cloud 66 by calling them in the [manifest file](/:product/:version?/manifest/what-is-a-manifest-file) of your application. You can set a different profile for each component of an application (e.g. MySQL or Redis). We will then use that profile whenever we provision a server for that component.

### Required AWS permissions

Attaching an instance profile requires three permissions on the IAM user behind your [cloud key](https://app.cloud66.com/clouds):

- `iam:PassRole`
- `ec2:AssociateIamInstanceProfile`
- `ec2:ReplaceIamInstanceProfileAssociation`

All three are already granted by our [recommended minimum policy](https://help.cloud66.com/c66_aws_iam_policy.json), where the two EC2 actions are covered by `ec2:*`. If you have narrowed that policy to suit your own security requirements, check that these survived. Instance profiles are the only feature that needs `iam:PassRole`, so it is an easy one to drop.

<Callout type="warning" title="Missing permissions fail at provisioning time">
...
1. Attach the role directly in the AWS console. Select the instance under *EC2*, then *Actions* → *Security* → *Modify IAM role*, and choose your profile. This takes effect immediately and does not require a restart.
2. Remove the server and scale up again. Any new server is created with the profile attached.

<Callout type="warning" title="Replacing a server disrupts traffic">
...
This requires Cloud 66 to have IAM permissions on your AWS account, so please be sure to set them up as explained in our [Deploying Your First App](/:product/:version?/getting-started/deploy-your-first-app) guide.
