...
If your gem needs any custom environment variable you can set them in a number of ways. Please read [our full guide to env-vars](/:product/:version?/build-and-config/env-vars) to learn more.
...
1. Add a new [environment variable](/:product/:version?/build-and-config/env-vars) to your application with an appropriate name (e.g `SIDEKIQ_USER_PASS`) - the value of the variable must use the format `username:password` 
