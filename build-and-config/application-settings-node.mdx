...
lead: >-
  How to customize the deployment configuration of your Node.js application via
  Cloud 66
...
During the analyze phase we analyze your runtime depencies defined in the `package.json`. Inclusion of the following packages will result in relevant datasource to be provisioned on your application.
...
None of the files created on the filesystem after the application is deployed are persistent. If you need persistency for files, for example some uploaded data, [please use GlusterFS](/node/servers/glusterfs).
