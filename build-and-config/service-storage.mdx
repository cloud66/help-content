...
This guide applies to containerized applications. We have a separate guide for Rails applications that deals with [persistent and shared directories](/deploy/:version?/build-and-config/rails-shared-directories).
</PerProduct>
...
You can choose whether to populate the mounted folder from a Secret or ConfigMap - the contents of which are automatically generated from your application [ConfigStore](/deploy/:version?/build-and-config/config-store) or [Environment Variables](/deploy/:version?/build-and-config/env-vars). 

Optionally you can filter the included values based on a tag/metadata filter. The filter must be of the syntax `key=value` ie. `foo=bar`. When applied to ConfigStore entries, it will match based on the entry metadata. When applied to Environment Variables, it will match based on [tags](/deploy/:version?/servers/tagging-components).
