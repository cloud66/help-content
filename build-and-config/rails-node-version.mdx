...
We automatically install the latest release of Node version <ComponentVersion names={["node"]} tabular={false} />  when we set up your Rack/Rails application servers. You can control which version is installed by editing the [manifest file](/docs/manifest/building-a-manifest-file) for the Rails application.
