...
You can also manage your environment variables using your [Cloud 66 Toolbelt](/:product/:version?/toolbelt/using-cloud66-toolbelt). There are four methods for managing env vars via Toolbelt:

- [Listing environment variables](/:product/:version?/toolbelt/_env-vars-list)
- [Downloading variables](/:product/:version?/toolbelt/_env-vars-download) as a file
- [Setting individual variables](/:product/:version?/toolbelt/_env-vars-set)
- Setting variables in bulk by [uploading a file](/:product/:version?/toolbelt/_env-vars-upload)
...
Note that the key name **must** be capitalized. 
...
The environment variable you are calling **already exists** in your Cloud 66 application. If it doesn't, this call will result in a build error. 
