...
2. Add a `PASSENGER_ENTERPRISE_DOWNLOAD_TOKEN` [environment variable](/:product/:version?/build-and-config/env-vars) which contains the value of your Passenger Enterprise download token.
...
We recommend that you choose Passenger Enterprise as your web server at the time of **initial build** of the application. If you already have an application, please [scale up](/:product/:version?/servers/scaling) to a new application server and scale down the old one to add Passenger Enterprise.
</Callout>