...
4. The new server joins your existing cluster — as a manager (control-plane node) or a worker, depending on what you chose.
...
Cloud 66 marks the selected servers for removal and deletes them in a single operation. Servers running database workloads are excluded from automatic scale-down to protect data; if you try to scale below the safe number you'll see:
