...
* MySQL (or Percona if [configured via Manifest](/:product/:version?/manifest/building-a-manifest-file#mysql))
* Postgres
* MongoDB
* Redis
* InfluxDB
...
This hook simply overwrites the standard config file with the config file of your choice at the start of the deployment process. Please read our [full guide to deploy hooks](/:product/:version?/deploy-hooks/using-deploy-hooks) to learn how to implement this hook.
</PerProduct>
<PerProduct includes={["rails"]}>
...
<PerProduct includes={["deploy"]} excludeVersions={['1']}>
...
To connect to a database use the ElasticDNS address. Please read our [full guide on ElasticDNS](/deploy/1/networking/service-networking#elasticdns) for more details.
</PerProduct>
...
You can also configure an external database via your [Manifest file](/:product/:version?/manifest/building-a-manifest-file#manifest-settings-for-databases#specifying-external-databases-via-your-manifest) by specifying the `server` node as `external`. 
...
You can make a new instance of your application by [cloning it](/:product/:version?/cloud-66-101/adding-updating-deleting#clone-an-application). We will provision new components for your application, which will use the latest versions by default. Be sure to check that you have not locked any component versions using your [Manifest file](/:product/:version?/manifest/building-a-manifest-file). 
...
[Toolbelt](/:product/:version?/toolbelt/toolbelt) which gives you the option of running migrations or not.

```shell
$ cx settings set -s my_stack run.deploy.command true
```

When you have disabled `run.deploy.command` in [Application settings](/:product/:version?/toolbelt/toolbelt#settings-set), you still have the option to run migrations on a one-off deployment by clicking _Deploy_ -> _Deploy with options_ and selecting _Run database migrations_.
...
- Updating your [Manifest file](/:product/:version?/manifest/building-a-manifest-file#manifest-settings-for-databases) will not be sufficient to reconfigure your application - you will need to connect your application manually to your new database servers, including authentication credentials, ideally via your app’s [environment variables](/:product/:version?/build-and-config/env-vars).
