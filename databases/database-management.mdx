...
* MySQL (or Percona if [configured via Manifest](/:product/:version?/manifest/building-a-manifest-file/_mysql)
* Postgres
* MongoDB
* Redis
* InfluxDB
...
You can also configure an external database via your [Manifest file](/:product/:version?/manifest/building-a-manifest-file/_manifest-settings-for-databases#specifying-external-databases-via-your-manifest by specifying the `server` node as `external`.
