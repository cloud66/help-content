...
- You can easily [restore](/:product/:version?/databases/manage-backups) or download database backups using the Dashboard or [Toolbelt](/:product/:version?/toolbelt/toolbelt#backups-download) (CLI)
- [Backup verifiers](/:product/:version?/databases/backup-verifiers) ensure that your backups actually contain what you expect
- Allows you to use [database replication](/:product/:version?/databases/database-replication) to scale your databases
...
If you are confident that you have enough space and the first option is not possible, you can use the [Cloud 66 Toolbelt](/:product/:version?/toolbelt/toolbelt#settings-set) to run `"cx settings set -s STACK_NAME db.check.backup.size false"` where `STACK_NAME` is the name of your application. This will disable backup size checks.
...
If you need to manually download and/or restore a backup, please read our in-depth guide on [manually restoring database backups](/:product/:version?/databases/manage-backups#manually-restoring-a-backup).
...
If you have [added](/:product/:version?/databases/adding-database#adding-logical-databases-to-a-server) any <Hint caption="logical databases">i.e. the tables & data structures rather than the "physical" database server ([full definition](#logical-databases-vs-physical-databases) </Hint> to any Database Group(s) you should be aware of the following:
