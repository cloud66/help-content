...
- You can easily [restore](/:product/:version?/databases/manage-backups) or download database backups using the Dashboard or [Toolbelt](/:product/:version?/toolbelt/_backups-download) (CLI)
...
If you are confident that you have enough space and the first option is not possible, you can use the [Cloud 66 Toolbelt](/:product/:version?/toolbelt/_settings-set) to run `"cx settings set -s STACK_NAME db.check.backup.size false"` where `STACK_NAME` is the name of your application. This will disable backup size checks.
