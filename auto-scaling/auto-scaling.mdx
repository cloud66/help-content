...
- [RabbitMQ](/:product/:version?/servers/rabbitmq)

</PerProduct>

<PerProduct includes={["deploy"]}>

In order to use queue size as a metric, you need to [add RabbitMQ](/:product/:version?/servers/rabbitmq) to your application and actively use it as your message queue. 
...
When servers are scaled down, they will not be automatically deleted unless you have have changed the [related setting](/:product/:version?/servers/server-deletion) for your application
</Callout>
