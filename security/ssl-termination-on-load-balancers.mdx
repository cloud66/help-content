...
If you need help getting started with SSL and certificates on Cloud 66 [follow our detailed SSL guide](/:product/:version?/security/ssl).
...
Ensure that the certificate is valid. The current date must be between the certificate’s start and end dates. You must also [remove any passphrases from your certificate keys](/:product/:version?/security/remove-passphrase).
</Callout>
