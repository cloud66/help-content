...
Your application's WAF is a **separate and different component** from the traditional (IP-table-based) [firewall](/:product/:version?/security/firewall-rule) that we install on each server we provision. The WAF allows you to set up a richer and more customisable set of access rules for your application's components than would be possible with IP tables.

<Callout type="info" title="Nginx may need to be updated">
 Older applications will need to apply an [application update](/:product/:version?/build-and-config/application-updates) to install the latest version of NGINX and the components that enable WAF. 
</Callout>

If the only rule you need to apply is to force all traffic to your servers to flow through your load balancer(s) then you can find that rule under [Traffic Filters](/:product/:version?/networking/network-configuration).
...
1. Manually, via the ModSecurity [CustomConfig](/:product/:version?/custom-config/custom-config) file
...
If the rule-set you added does have "relevant configuration sections", you should now review and update the OWASP [CustomConfig](/:product/:version?/custom-config/custom-config) file accordingly. 
