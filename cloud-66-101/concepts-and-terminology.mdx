...
...as well as all the configuration files, settings and environment variables on which these components rely.
...
Your servers will be deployed with <OperatingSystemsWrapper showAll={false} humanReadable={true} />.

### Cloud vendor

You can either [deploy to your preferred cloud provider](/:product/:version?/specs-and-policies/technical-specifications#supported-cloud-providers) <PerProduct includes={["rails", "deploy"]}>or [deploy to your own server](/:product/:version?/servers/registered-servers)</PerProduct>.
...
BuildGrid is Cloud 66's integrated container image builder. Whenever you build images from source code, or retag existing images, you make use of a metered resource called Build Minutes. All Cloud 66 accounts have a number of “free” Build Minutes per month.
...
Your servers will be deployed with <OperatingSystemsWrapper />.
