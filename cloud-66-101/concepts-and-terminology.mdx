...
...as well as all configuration files, settings and environment variables on which these components rely.
...
Your servers will be deployed with **Ubuntu 22.04 LTS**.
...
The first time Cloud 66 provisions a new Kubernetes cluster, we default to the **shared master** architecture. This can be changed via the [Dashboard](/:product/:version?/servers/scaling#kubernetes-cluster-servers) or [manifest.yml](/:product/:version?/manifest/building-a-manifest-file).
...
- The [Manifest file](/:product/:version?/manifest/building-a-manifest-file#manifest-tutorial) captures settings for infrastructural components in a simple YAML format, making it quick and easy to roll out additional instances of a component without any manual intervention
