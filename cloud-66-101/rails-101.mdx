...
3. **Add a cloud provider.** Connect us to your preferred cloud provider, or use one of our [free credit](/:product/:version?/cloud-66-101/free-google-cloud-credits) offers.
4. **Deploy your app!**
...
You can specify which server will be used for the migration task using [reserved tags](/:product/:version?/servers/reserved-tags). You can turn off automatic migrations using [Toolbelt](/:product/:version?/toolbelt/toolbelt#settings-set). You still have the option to run migrations on a one-off deployment via your Dashboard by clicking *Deploy* -> *Deploy with options* and selecting *Run database migrations*.
...
- In your Manifest file (*[what is a Manifest file?](/:product/:version?/manifest/building-a-manifest-file#what-is-a-manifest-file)*) - for apps that are already deployed
...
If you’d prefer to use your chosen toolchain to entirely replace Asset Pipeline Compilation, you can deactivate Asset Pipeline Compilation it in your Manifest file (*[what is a Manifest file?](/:product/:version?/manifest/building-a-manifest-file#what-is-a-manifest-file)*).
