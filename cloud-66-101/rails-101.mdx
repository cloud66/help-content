...
- Toolbelt - our [command line tool](/rails/toolbelt/using-cloud66-toolbelt)

If you’re planning to use Toolbelt (we recommend it!), you will need to [install it via your terminal](/rails/toolbelt/using-cloud66-toolbelt#install-toolbelt). 

## Get going

We’ve made deploying an app as quick and easy as possible, so you should have your Rails app deployed in just a few minutes. Just follow the steps below. We have a separate [getting started guide](/rails/getting-started/deploy-your-first-app) which has more detail if you need it.
...
3. **Add a cloud provider.** Connect us to your preferred cloud provider, or use one of our [free credit](/rails/cloud-66-101/free-google-cloud-credits) offers.
4. **Deploy your app!**
...
- [Thin](/rails/build-and-config/thin-rack-server)
- [Puma](/rails/build-and-config/puma-rack-server)
- [Unicorn](/rails/build-and-config/unicorn-rack-server)
...
You can specify which server will be used for the migration task using [reserved tags](/rails/servers/reserved-tags). You can turn off automatic migrations using [Toolbelt](/rails/toolbelt/toolbelt#settings-set). You still have the option to run migrations on a one-off deployment via your Dashboard by clicking *Deploy* -> *Deploy with options* and selecting *Run database migrations*.
...
For more commands, see our [Toolbelt guide](/rails/toolbelt/toolbelt#env-vars-list).
...
- By default we will automatically run Asset Pipeline Compilation on each of your deploys. You can choose to [disable](/rails/deployment/enable-disable-asset-pipeline#enable-or-disable-apc) this.
- We choose one of your servers to handle the workload, leaving your other servers free to run your application as usual. You can also [nominate a preferred server](/rails/deployment/enable-disable-asset-pipeline#nominating-a-dedicated-compilation-server) to handle this workload.
...
- In your Manifest file (*[what is a Manifest file?](/rails/manifest/building-a-manifest-file#what-is-a-manifest-file)*) - for apps that are already deployed
...
If you’d prefer to use your chosen toolchain to entirely replace Asset Pipeline Compilation, you can deactivate Asset Pipeline Compilation it in your Manifest file (*[what is a Manifest file?](/rails/manifest/building-a-manifest-file#what-is-a-manifest-file)*).
...
To use Action Cable, mount it in the application itself rather than as a separate process. For more about Action Cable, read our [separate guide](/rails/build-and-config/websocket-support). 
...
You can use deploy hooks ([*what are deploy hooks?*](/rails/deploy-hooks/deploy-hooks)) to execute your rake task at any point of your deployment.
...
You can install all of these using [Deploy Hooks](/rails/deploy-hooks/deploy-hooks#install-packages-on-each-server-deployed) (*[what are deploy hooks?](/rails/deploy-hooks/deploy-hooks)*). To install `libvips` for example, you would use hook code like this:
...
Modern Rails applications rely on a range of components to perform at scale. **Cloud 66 supports every Ruby and Rails component your application needs**, either natively or via our powerful [Deploy Hooks](/rails/deploy-hooks/deploy-hooks) feature. If it can run on Ubuntu, we support it. 
