...
You can run database migrations during deployment by invoking the artisan command-line tool via a deploy hook ([*what are deploy hooks?*](/deploy/:version?/deploy-hooks/deploy-hooks)). For example:
...
You can set all of these during your initial application config - see the section above for how to set env_vars for your app. It may be useful to use the [Auto Generate](/deploy/:version?/build-and-config/env-vars#using-auto-generate) feature for either the app key or the secret (or both)

Finally, in order to initialise the Echo server you need to run another shell command. You can do this via another simple deploy hook ([*what are deploy hooks?*](/deploy/:version?/deploy-hooks/deploy-hooks))
