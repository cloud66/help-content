...
If your application has two or more [database groups](/:product/:version?/databases/attaching-multiple-databases), then the ElasticDNS addresses for those groups will be derived from their names. For example if you have two MySQL database groups named `main` and `spare` then the ElasticDNS addresses will be:

- `main.mysql.cloud66.local`
- `spare.mysql.cloud66.local`

If one of these groups is set as the ["primary"](/:product/:version?/databases/attaching-multiple-databases#understanding-primary-database-groups), it will also be mapped to the default mysql group (`mysql.cloud66.local`).
