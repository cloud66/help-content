---
title: Service networking configuration (CSv1)
legacy: true
---

{% callout type="error" title="Legacy feature" %}
PLEASE NOTE: This guide only applies to Version 1 of Cloud 66 Container Service, also know as *CSv1*. If you are new to Cloud 66 then you should use the Dashboard features to manage this configuration.
{% /callout %}
...
If your application has two or more [database groups](/docs/databases/attaching-multiple-databases), then the ElasticDNS addresses for those groups will be derived from their names. For example if you have two MySQL database groups named `main` and `spare` then the ElasticDNS addresses will be:

- `main.mysql.cloud66.local`
- `spare.mysql.cloud66.local`

If one of these groups is set as the ["primary"](/docs/databases/attaching-multiple-databases#understanding-primary-database-groups), it will also be mapped to the default mysql group (`mysql.cloud66.local`).
