...
Start by checking if you can SSH to your server - follow our [SSH guide](/:product/:version?/servers/ssh-to-server) if you need help. If you’re able to SSH to the server, try the steps below. If not, we recommend that you use your cloud vendor dashboard to try to connect to the server. Our separate guide to [troubleshooting server issues](/:product/:version?/servers/troubleshooting-server-connection) may be of some help here. 

## Run a Health Check

You can run a [quick health check](/:product/:version?/servers/application-health-checks#global-availability-checks) from your Cloud 66 dashboard. This will tell you if your application is responding to HTTP requests from our testing servers in four different regions. This will reveal (or rule out) any network connectivity issues between your own computer and your application.
...
Additionally, an *immediate bounce* of this command indicates that there is no server listening, whereas a more *lengthy response* could indicate a [firewall](/:product/:version?/security/firewall-rule) issue.
...
It may be worth checking your [Nginx CustomConfig](/:product/:version?/custom-config/custom-config#accessing-custom-config) history to see if any recent configuration changes are causing issues.
