...
In order to manually query a server's metadata, you must first [log into the server via SSH](/:product/:version?/servers/ssh-to-server). Once you are logged in, you can use the following command:
...
#### Sample for Cloud 66 applications
