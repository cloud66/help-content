...
Deployment Success Checks are defined either in your application’s **[manifest file](/:product/:version?/manifest/building-a-manifest-file#manifest-tutorial)** or via the Dashboard.
...
The values above are the default values and are all optional. Any options that are excluded will simply use the defaults above. You can see a complete list of supported options in our **[detailed manifest guide](/:product/:version?/manifest/building-a-manifest-file)**.
...
2. Via your Manifest file (please see the [Manifest reference guide](/:product/:version?/manifest/building-a-manifest-file#post-deployment-health-checks))
