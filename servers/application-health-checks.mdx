...
There may occasionally be a lag between adding web servers back to a load balancer during a [phased deployment](/:product/:version?/deployment/parallel-deployment) (e.g. rolling) and the load balancer completing its own health checks, meaning that traffic is actually flowing to the server.
...
- We check if [systemd](/:product/:version?/servers/systemd) is reporting that your application is healthy
...
Deployment Success Checks are defined either in your application’s **[manifest file](/:product/:version?/manifest/what-is-a-manifest-file)** or via the Dashboard.
...
The values above are the default values and are all optional. Any options that are excluded will simply use the defaults above. You can see a complete list of supported options in our  [detailed manifest guide](/:product/:version?/manifest/what-is-a-manifest-file).
...
If your application does not have a valid endpoint or route set for `/` then you can specify a custom path under the `httpchk` option in your  [manifest file](/:product/:version?/manifest/_manifest-settings-for-load-balancers)  to ensure your application responds appropriately. We will configure the load balancer to ping that path rather than the root.
...
2. Via your Manifest file (please see the [Manifest reference guide](/:product/:version?/manifest/_health-checks)
...
Service Checks are configured via your `service.yml` file. For more information please read our separate guide to setting up [Service Checks in Cloud 66](/:product/:version?/build-and-config/service-lifecycle-management#health).
</PerProduct>