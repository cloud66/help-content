...
Server Jobs allow you to define and schedule regular server tasks (shell commands). Under the hood, these are run as [cronjobs](/:product/:version?/servers/understanding-cron-syntax).

<PerProduct includes={["django", "expressjs", "nextjs", "node", "laravel"]}>
If you need to run a job against a Cloud 66 service (i.e. inside a container), rather than on the underlying server, you should use [Application Jobs](/:product/:version?/servers/docker-tasks) instead.
</PerProduct>
...
When you are running a job on demand via dashboard or [toolbelt](/:product/:version?/toolbelt/toolbelt#jobs-run), you can pass it parameters (if it is written to accept them).
