...
If you haven't deployed your application in a production environment yet, you can deploy to a reasonably sized server and use [load testing](/:product/:version?/servers/load-testing) to determine your exact needs.

## Under-powered server sizes (not recommended)

The following server sizes **are not recommended** (especially not in production). Some of these may only be available via [Registered Servers](/:product/:version?/servers/registered-servers).
