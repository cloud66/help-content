...
The **owner** of an account always has the maximum permissions for all applications and does not need to be assigned any roles. To transfer the owner role to another user, see [Changing the account owner](/:product/:version?/account/change-owner).
