...
products: all
lead: >-
  You can have your first application up and running on Cloud 66 in under 5
  mins. Follow the steps below to see how easy it is to get your app configured,
  built, and deployed.
category: getting-started
---
import Git from '@/docs/partials/_git.mdx'
import PerProduct from "@/components/PerProduct"
import Callout from "@/components/ui/callout"
import CloudProviderOrOwnServer from "@/docs/partials/_cloud_provider_or_own_server_tabs.mdx"
import ObjectStorage from "@/docs/partials/_object_storage.mdx"
...
<PerProduct includes={["deploy"]}>
    If you don't have images or code ready, you can use this [simple visit counter
    application](https://github.com/cloud66/maestro-demo.git) we've created on Github.
</PerProduct>
...
<PerProduct includes="node">
    <Callout type={"warning"} title={"Cloud 66 for Node"}>
    Cloud 66 Node is no longer supported. Please use Cloud 66 Deploy for your node applications.
    </Callout>
</PerProduct>
...
### Add and confirm environment variables

<Pill title="Optional" color="orange"/>
...
If we detect that your application uses Svelte we will ask you to confirm whether your application is **statically generated** (AKA pre-rendered) or uses **server - side rendering**.
...
If we detect that your application uses Next.js we will ask you to confirm whether your application is **statically generated** (AKA pre-rendered) or uses **server - side rendering**.
...
#### C.Set the desired number of instances
...
| --- | --- |
| Specific versions of a framework or package | [Manifest file](/:product/:version?/manifest/building-a-manifest-file) |
| Cloud-provider-specific settings for servers (e.g. security groups, subnets) | [Manifest file](/:product/:version?/manifest/building-a-manifest-file) |
...
| Running multiple database types or versions (groups) | [Manifest file](/:product/:version?/manifest/building-a-manifest-file) or Dashboard |
