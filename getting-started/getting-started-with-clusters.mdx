...
import CloudProviderOrOwnServerTabs from '@/docs/partials/_cloud_provider_or_own_server_tabs.mdx'

A Cloud 66 Cluster is a "blank" instance of Kubernetes infrastructure - i.e. with no application specific components installed. You can use a Cluster to deploy multiple applications to the same set of servers.
...
You can also use your own server - although you will need to [register it](/deploy/:version?/servers/registered-servers#register-a-server) first. In our example we're choosing to deploy to Digital Ocean.
...
<PerProduct excludeVersions={["3"]} product="deploy">
    <Callout type="info" title="First server is always the master node">
        The first server in your cluster will always be your Kubernetes master node. You can decide later if you would like\n    this server to share application workloads or only run Kubernetes management tasks.
    </Callout>
</PerProduct>
...
* Learn how to add a [load balancer](/deploy/:version?/load-balancers/load-balancer) to your application.
* Learn about using [failover groups](/deploy/:version?/failover-groups/failover-groups) to make your application highly available
* Learn how to [deploy your service(s) in parallel](/deploy/:version?/deployment/parallel-deployment) to speed up the deployment process.
* Your deployment configuration is stored in a *manifest.yml* file. Learn how to [edit your manifest file](/deploy/:version?/manifest/building-a-manifest-file) to access advanced deployment features.
