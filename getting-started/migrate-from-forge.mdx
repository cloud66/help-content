...
If you’d prefer to use another method, you can consult the table on our [main getting started guide](/deploy/:version?/getting-started/deploy-your-first-app#step-1-choose-a-source).
...
5. You can also define a [persistent storage volume](/deploy/:version?/build-and-config/service-storage) if needed
...
3. Specify whether your datastore will share the app server (not recommended for Production), or have its own server. You can also use an existing [external database server](/deploy/:version?/databases/database-management#no-database-external) if you prefer.
...
* Get started with [manifest files](/deploy/:version?/manifest/building-a-manifest-file#manifest-tutorial) - a powerful tool for defining your application's components
* Learn about [CustomConfig](/deploy/:version?/custom-config/custom-config) - a tool for defining and managing configuration templates
* Learn how to use [Toolbelt](/deploy/:version?/toolbelt/using-cloud66-toolbelt) - a powerful command-line interface for managing your Cloud 66 applications. 
