...
This gives you a file with all your config vars in `KEY=value` format. You can then add these to your Cloud 66 application via the [environment variables](/:product/:version?/deployment/environment-variables) page on the dashboard, or using the [Cloud 66 toolbelt](/:product/:version?/toolbelt/using-cloud66-toolbelt).

<Callout type="warning" title="Review before copying">
