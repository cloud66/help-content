...
Traffic allows you to define arbitrarily complex rules to route and modify traffic to your Sites application. If you need help writing, testing or implementing Traffic rules, please read our [how-to guide](/:product/:version?/networking/using-traffic-rules-to-route-traffic). This reference guide describes rule functions and their parameters in more detail for those who need to write more complex rules.
...
Traffic supports string functions such as `replace`, `join` and `LowerAscii`.  We have a [separate reference doc](/:product/:version?/networking/traffic-cel-functions) that lists all of them.
