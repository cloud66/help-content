...
Traffic supports string functions such as `replace`, `join` and `LowerAscii`.  We have a [separate reference doc](/sites/networking/traffic-cel-functions) that lists all of them.
