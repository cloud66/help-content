...
If you need help with service networking for containerized applications, please read our [Service Networking for Containers](/:product/:version?/networking/service-networking) guide.
</PerProduct>
...
* **An existing application set up in Cloud 66** &mdash; To make the most of this tutorial you need to have an app already set up in Cloud 66. Follow our [Getting Started guide](/:product/:version?/getting-started/deploy-your-first-app) if you're not sure how to do this. 
...
Cloud 66 supports a wide range of configuration customizations for Nginx. You can read our [in-depth reference guide](/:product/:version?/servers/nginx) for more details.

All changes to configuration files in Cloud 66 are automatically tracked and version controlled by CustomConfig git. Read [our guide](/:product/:version?/custom-config/custom-config) to better understand the power of this feature.
...
You use the Cloud 66 [SSL feature](/:product/:version?/security/ssl) to add a certificate to your application and serve your traffic securely via HTTPS. To ensure that all your visitors use HTTPS, you should redirect anyone using HTTP to HTTPS.
...
If you have previously specified CORS settings in your [Manifest file](/:product/:version?/manifest/building-a-manifest-file#cors-configuration), we will use these settings in the interface described below.
...
You can also managed these settings via your [Manifest file](/:product/:version?/manifest/building-a-manifest-file#cors-configuration).

## Application Surge Protection

To help prevent denial of service (DOS) attack, Cloud 66 automatically blocks any IP address that makes more than 1,500 requests per minute to your server(s). We call this Surge Protection. You can see if any IP addresses are currently being block by clicking on the [Active Protect](/:product/:version?/security/active-protect) tab on your application's **Home** page.
...
Please read our [separate guide](/:product/:version?/security/web-application-firewalls) for details on WAF.

### OWASP Rules

Please read our [separate guide](/:product/:version?/security/web-application-firewalls#using-owasp-rules-with-your-waf) for details on OWASP rules for WAF.

## What’s next?

* Learn how to use [CustomConfig](/:product/:version?/custom-config/custom-config) to manage the configuration files for components like Nginx 
* Learn how to [configure your public DNS](/:product/:version?/networking/configure-dns) to work optimally with Cloud 66
* Learn how to [add a load balancer](/:product/:version?/networking/configure-dns) to your application 
