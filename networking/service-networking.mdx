...
Port mapping is a small (but important) part of a core system that underpins container management - *service networking*. If you need an introduction to the concept of Service Networking, you can find one [here](/deploy/:version?/cloud-66-101/concepts-and-terminology#service-networking).
...
* **An existing containerized application set up** &mdash; To make the most of this tutorial you need to have an app already set up in Cloud 66. Follow our [Getting Started guide](/deploy/:version?/getting-started/deploy-your-first-app) if you're not sure how to do this.
...
When we set up our *demo-app* service we [configured our ports](/deploy/:version?/getting-started/deploy-your-first-app#configure-your-services) so that our internal port `5000` was mapped to the public port `80`. 
...
You can also read [our guide to using service.yml](/deploy/:version?/build-and-config/docker-service-configuration) for more help.
...
<Tabs defaultValue="container-service-v2">
    <TabsList>
        <TabsTrigger value="container-service-v2">Container Service V2</TabsTrigger>
        <TabsTrigger value="container-service-v1">Container Service V1</TabsTrigger>
    </TabsList>
    <TabsContent value="container-service-v2">
...
If you've followed the steps above correctly, the **Network** column will list both port *80* and port *443*.
    ...
    <TabsContent value="container-service-v1">
...
If you actually need HTTPS traffic to be available to the outside world (not just as a demo) you will also need to set up [SSL certificates](/deploy/:version?/security/ssl) for your application.
</Callout>
...
2. Via the [Configuration Files interface](/deploy/:version?/build-and-config/docker-service-configuration#getting-started-serviceyml) 
...
This section covers more complex and advanced cases of service networking for containerized applications. If you've never configured a containerized service before, consider following [our tutorial](/deploy/:version?/networking/service-networking#mapping-container-ports) on port mapping first.

If you need an introduction to the concept of Service Networking, you can find one [here](/deploy/:version?/cloud-66-101/concepts-and-terminology#service-networking).

Read [our guide to using service.yml](/deploy/:version?/build-and-config/docker-service-configuration) for more help on customizing your service configuration.
