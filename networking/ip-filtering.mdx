...
Cloud 66 also automatically rate limits IP addresses and will temporarily block an address that hits your application too often. Please read our [Application Surge Protection](/:product/:version?/networking/network-configuration#application-surge-protection) guide for more details.
...
Whitelisting is not the same as removing [Application Surge Protection](/:product/:version?/networking/network-configuration#application-surge-protection). These are two separate lists with separate functions. A whitelisted IP could still be rate limited.
</Callout>
...
You can also add lists of addresses via URLs. The URL must point to either a `txt` or `JSON` formatted document. See our [reference guide](/:product/:version?/networking/network-configuration#traffic-filters) for more details. 
...
As above, you can enter IP addresses in comma separated lists, as ranges, or a combination. You can also add lists of addresses via URLs. The URL must point to either a `txt` or `JSON` formatted document. See our [reference guide](/:product/:version?/networking/network-configuration#traffic-filters) for more details. 
...
We automatically provision (unique) [default (sub)domains](/:product/:version?/networking/configure-dns#cloud-66-hostnames) for every application under `c66.me`. You can control whether this domain is directly reachable or not (it is reachable by default).
...
**Blocking** direct traffic will return a 404 to visitors to that domain, and display [custom error page](/:product/:version?/servers/nginx#customizing-nginx-configurations) you have configured. 
